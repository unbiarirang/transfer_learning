...
parser.add_argument('-a', '--is-all', default=False, required=False)
parser.add_argument('--save-path', default='./checkpoint-cn/model.pt', required=False, help='checkpoint save path')
parser.add_argument('--load-path', default='./checkpoint-cn/model.pt', required=False, help='checkpoint load path')
parser.add_argument('-s', '--is-save', default=False, required=False)
parser.add_argument('-l', '--is-load', default=False, required=False)
parser.add_argument('-t', '--is-train', default=True, required=False)
...
is_all = args.is_all # Use all train and test data
...
class LSTMTagger(nn.Module):
    ...
    def __init__(self, word_dict, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        self.word_embeddings = get_embeddings(word_dict, 'wiki_cn.vec', embedding_dim)
...
def main():
    ...
    train_objs = train_objs[:len(train_objs) - len(test_objs)] # 4/5 of train set
    ...
    with open(LOG_FILE, 'a') as f:
        f.write('embedding-dim: {} hidden-dim: {} epoches: {} train_size: {} test_size: {} accuracy: {}% is-train: {} cp: {}\n'.format( \
            EMBEDDING_DIM, HIDDEN_DIM, EPOCHES, \
            TRAIN_SIZE if not is_all else len(train_objs), \
            TEST_SIZE if not is_all else len(test_objs), accuracy, \
            is_train, "" if not is_load else LOAD_PATH))
